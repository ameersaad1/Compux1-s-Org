...
export function CompuxLogo({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none" className="shrink-0">
      <defs>
        <linearGradient id="compux_lg1" x1="0" y1="0" x2="40" y2="40" gradientUnits="userSpaceOnUse">
      ...
      />
      <path
        d="M26 15L32 21M32 15L26 21"
        stroke="url(#compux_lg1)"
        strokeWidth="2.8"
        strokeLinecap="round"
      />
    </svg>
  ...
  return (
    <div className="relative shrink-0 inline-block" style={{ width: size, height: size }}>
      <img
        src={src}
        alt={name}
        className="rounded-full object-cover w-full h-full shadow-xs"
        style={{ border: "2px solid rgba(109,94,245,0.35)" }}
        loading="lazy"
      />
      {online && (
        <span
          className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-400 ring-2 ring-white dark:ring-neutral-900"
        />
      )}
    </div>
  );
...
export function VerBadge({ color = "#7c3aed", size = 14 }: { color?: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none" className="shrink-0 inline-block" title="Verified Campus Student">
...
export function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      type="button"
      onClick={onChange}
      className="relative w-11 h-6 rounded-full transition-all shrink-0 focus:outline-hidden"
      style={{ background: checked ? "var(--primary)" : "var(--muted)" }}
      aria-checked={checked}
      role="switch"
    >
      <span
        className="absolute top-1 w-4 h-4 rounded-full bg-white shadow-xs transition-all"
...
export function GlobalToast() {
  ...
  return (
    <div
      className="fixed top-5 left-1/2 -translate-x-1/2 z-[300] px-5 py-3 rounded-2xl text-sm font-semibold text-white gradient-bg glow pointer-events-none transition-all"
      style={{ fontFamily: "'Outfit', sans-serif", boxShadow: "0 8px 32px rgba(109,94,245,0.4)" }}
    >
      ✓ {toast}
    </div>
  );
...
export function EmptyState({ emoji, title, sub }: { emoji: string; title: string; sub?: string }) {
  return (
    <div className="glass rounded-2xl p-12 flex flex-col items-center text-center w-full" style={{ border: "1px solid var(--border)" }}>
